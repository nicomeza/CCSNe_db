...
uvot = np.genfromtxt('/home/dustspeck/SN/UVOT_zeropoint.dat',usecols=[0,1,2],dtype={'names':('filter','lambda','flux'),'formats':('S5','f8','f8')})
vega = np.genfromtxt('/home/dustspeck/SN/AB_zeropoints.dat',usecols=[0,1,2,3],dtype={'names':('filter','lambda','flux','ZP'),'formats':('S5','f8','f8','f8')})
uvot_AB = np.genfromtxt('/home/dustspeck/SN/UVOT_AB.dat',usecols=[0,1,2,3],dtype={'names':('filter','z','z_err','AB_V'),'formats':('S5','f8','f8','f8')})
#print uvot_AB['filter'],uvot_AB['AB_V']

vega['lambda']*=1e4 # Transforming wavelength to Angstroms

uvot = np.ma.masked_array(uvot)
vega = np.ma.masked_array(vega)
...
lcogt_filters = ['U','B','V','R','I','u','g','r','i','z','J','H','K']
lcogt_names = ('U','B','V','R','I','u','g','r','i','z','J','H','K')
uvot_filters = ('W2', 'M2', 'W1', 'U', 'B', 'V')
uvot_names =  ('W2_uvot', 'M2_uvot', 'W1_uvot', 'U_uvot', 'B_uvot', 'V_uvot')
...
for SN,z_SN,E_B_V,t_0 in SN_DATA[['sn','sn_z','sn_ebv','t_0']]:
    ...
    try: 
        ...
        for filter in zip(filters):
            ...
            if os.path.isfile(filter_file):
                ...
                try:
                    ...
                    try:
                        uvot.mask[uvot_names.index(filter)] = (False,False,False)
                        print "filter %s added"%filter
                    except:
                        try:
                            vega.mask[lcogt_names.index(filter)] = (False,False,False,False)
                            print "filter %s added"%filter
                        except:
                            print "filter %s could not by added"%filter
                    ...
                    try:
                        uvot.mask[np.where(uvot['filter']==uvot_filters[uvot_names.index(filter)])[0]] = (False,False,False)
                        print "filter %s added"%filter
                    except:
                        try:
                            vega.mask[np.where(vega['filter']==lcogt_filters[lcogt_names.index(filter)])[0]] = (False,False,False,False)
                            print "filter %s added"%filter
                        except:
                            print "filter %s could not by added"%filter
        ...
        LCOGT = vega[np.in1d(vega['filter'],lcogt_filters)]  
        print LCOGT['filter']
        ...
        # Obtain filter with minimum cadence (less observed band). 
        # Plot derredened light curves
        ...
        for filter in uvot:
            ...
            mag = scope['mag_%s'%filter_name] - R_x*E_B_V   # Deredenned magnitudes 
        ...
        # Obtain magnitudes at the epoch of the band with less observations. 
        # Interpolate linearly if neccesary. 
        ...
        # Ordenar segun longitud de onda
        ...
        # Tomar magnitudes y pasar a escala AB y a flujo usando ZPs.
        # Plotear las magnitudes nuevamente
        ...
        for filter in LCOGT:
            ...
            lco_name = lcogt_names[lcogt_filters.index(lco_filter)]
            print lco_filter
            print filter['lambda']
        ...
        sorted_lambdas_lco = np.sort([LCOGT['lambda'][lcogt_names.index(filters_used[i])] for i in range(len(filters_used))])
        ...
        for filter,mags,errs in sorted_mags_lco:
            ...
            print filter
            index = lcogt_names.index(filter)
        ...
        filter_names_uvot = uvot['filter'][np.in1d(sorted_lambdas_uvot,uvot['lambda'])]
        filter_names_jc = vega['filter'][np.in1d(sorted_lambdas_lco,vega['lambda'])]
        filter_names = np.concatenate([filter_names_uvot,filter_names_jc],axis=0)
        ...
        for lam,flux in zip(sorted_lambdas,fluxes):
                
            ax.axvline(lam,linestyle='--',color='k')
            ax.text(lam,1.1*(flux*c_cms/((lam*1e-4)**2.)),band)

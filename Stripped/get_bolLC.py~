...
LCOGT = LCOGT_data[np.in1d(LCOGT_data['filter'],lcogt_filters)]  
...
vega = np.genfromtxt('/home/dustspeck/SN/Vega_zeropoints.dat',usecols=[0,1,2],dtype={'names':('filter','lambda','flux'),'formats':('S5','f8','f8')})
...
ax.legend(loc='best',ncol=2,prop={'size':9})
fig.savefig('./SED_lam_%s.png'%(R_V))
pl.show()
pl.close()
